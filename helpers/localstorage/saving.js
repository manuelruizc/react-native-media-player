    ...
    if (value == null) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          let config = {
            downloadTitle: title,
            downloadDescription: "Descargando cancion",
              ...
              AsyncStorage.setItem("Playlists", playlists)
                .then((asyncalive) => {
                  ...
                  AsyncStorage.setItem("Songs", songs)
                    .then((res) => {
                      let { downloadingVideoKey } = self.state;
                      downloadingVideoKey = downloadingVideoKey.filter(
                        (dld) => dld != songObject.uri
                      );
                      self.setState({ downloadingVideoKey });
...
export const createInitialPaths = async () => {
  let path_name =
    "file:///storage/emulated/0/Android/data/com.muustube/files/Download/";
  let path_imgs =
    "file:///storage/emulated/0/Android/data/com.muustube/files/Images/";
...
export const initialStorageSettings = async () => {
  const storage_data = await AsyncStorage.multiGet([
    "currentSong",
    "currentPlaylist",
    "Songs",
    "SourceIsAudio",
        ...
        if (source == "playlist") {
          if (currentPlaylistName != "songsDownloadedOnDevice") {
            playlist = playlist.filter((play) =>
              play.playlist.includes(currentPlaylistName)
            );
...
export const saveLastSongData = async (currentSongData) => {
  await AsyncStorage.multiSet([
    ["currentSong", JSON.stringify(currentSongData)],
    ["currentVideoItem", JSON.stringify(currentSongData)],
  ]);
};
