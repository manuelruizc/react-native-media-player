      ...
      try {
        await AsyncStorage.getItem("Songs")
          .then((response) => {
            ...
            if (songs != null) {
              let playlistSongs = songs.filter((song) =>
                song.playlist.includes(key)
              );
              ...
              let newSongsArray = songs.map((song) => {
                if (
                  song.uri == songObject.uri &&
                  !song.playlist.includes(key)
                ) {
                  song["playlistsIndex"] =
                    song["playlistsIndex"] === undefined
                      ? { [key]: newIndexPlaylist }
                      : { ...song["playlistsIndex"], [key]: newIndexPlaylist };
              ...
              if (!isSavedAlready) {
                songObject["playlistsIndex"] =
                  songObject["playlistsIndex"] === undefined
                    ? { [key]: newIndexPlaylist }
                    : {
                        ...songObject["playlistsIndex"],
                        [key]: newIndexPlaylist,
              ...
              songObject["playlistsIndex"] =
                songObject["playlistsIndex"] === undefined
                  ? { [key]: 0 }
                  : { ...songObject["playlistsIndex"], [key]: 0 };
    ...
    >
      ...
      >
        <TouchableOpacity onPress={goBack}>
          <Icon name={"ios-arrow-back"} size={28} color={"#EEEEEE"} />
        </TouchableOpacity>
        <Text style={{ color: "#EEEEEE", fontSize: 20 }}>Playlists</Text>
        <TouchableOpacity onPress={openModal}>
          <Icon name={"ios-add-circle-outline"} size={28} color={"#EEEEEE"} />
        </TouchableOpacity>
      </View>
      {playlists.length < 1 ? (
        <View
        ...
        <ScrollView style={{ width: "100%", backgroundColor: "#222222" }}>
          {playlists.map((play) =>
            play == "songsDownloadedOnDevice" ||
            play == "favorites__Playlist" ? null : (
              <TouchableOpacity
                onLongPress={() => openModalEdit(play)}
                onPress={() => addToPlaylist(play)}
                key={play}
                style={{
                  borderBottomWidth: 1,
                  borderColor: "#888888",
              ...
              >
                <Text style={{ color: "#EEEEEE" }}>{play}</Text>
              </TouchableOpacity>
            )
          )}
        </ScrollView>
      ...
      <View style={isModalActive ? styles.modalActive : styles.modalNotActive}>
        ...
        >
          ...
          >
            <Text
              style={{ color: "#EEEEEE", textAlign: "center", fontSize: 14 }}
            >
              Create playlist
            </Text>
          </TouchableOpacity>
