...
import { AsyncStorage, Dimensions, ToastAndroid, View } from "react-native";
...
import {
  __togglePause,
  __togglePlayerFullscreen,
  __updateAllSongs,
  __updateCurrentPlaylistName,
  __updateCurrentVideo,
  __updateCurrentVideoIndex,
  __updateCurrentVideoItem,
  __updateCurrentVideoKey,
  __updateDownloadedSongsList,
  __updateDownloadingVideoKey,
  __updateImageURI,
  __updateIsDownloadingSong,
  __updateLastSearch,
  __updateLoadingSearchStatus,
  __updatePlaylistSource,
  __updateRelatedVideos,
...
const App = () => {
  ...
  const musicPlayerFullScreen = useSelector(
    (state) => state.musicPlayerFullScreen
  );
  ...
  useEffect(() => {
    ...
    for (let i = tempDownloadingVideoKey.length - 1; i >= 0; i--) {
      ...
      if (exists) tempDownloadingVideoKey.splice(i, 1);
    }
  ...
  const changeSplashState = async () => {
    ...
    axios
      .get(`https://tubeplaya.herokuapp.com/search/${localStorageLastSearch}/`)
  ...
  const updateLastSongData = (data = null, index) => {
    ...
    AsyncStorage.multiSet([
      ...
      ["currentVideoItem", currentSongData],
    ])
      .then(function (response) {})
      .catch(function (error) {
        console.error(error);
  ...
  const playNextSongOnScroll = (
    ...
    if (video.isDownloaded) {
      ...
      changeImageURI(video.thumbnails[video.thumbnails.length - 1].url);
    ...
    axios
      .get(`https://tubeplaya.herokuapp.com/video_info/${id}`)
      .then(function (response) {
        ...
        changeCurrentVideo(
          sourceIsAudio
            ? response.data.formats[0].url
            : response.data.formats[2].url
        );
        changeCurrentVideoIndex(ix);
        changeCurrentVideoKey(id);
        ...
        let currentSongData = {
          [sourceIsAudio ? "pathAudio" : "pathVideo"]: sourceIsAudio
            ? response.data.formats[0].url
            : response.data.formats[2].url,
          key: id,
  ...
  const playNextSong = (
    ...
    newIndex =
      newIndex < 0 ? vidl.length - 1 : newIndex === vidl.length ? 0 : newIndex;
    if (index != null) newIndex = index;
    // if(newIndex === 0 && prev === null && index === null)

    const videoInfo =
      playlistSource != "playlist"
        ? videoList[newIndex]
        : videoListPlaylist[newIndex];

    changeCurrentVideoKey(videoInfo.uri);
    changeCurrentVideoIndex(nextIndex);
    changeSongProgress(0);
    let isSongPaused =
      newIndex === 0 && prev === null && index === null ? true : false;
    if (isPaused != null) {
    ...
    const currentList =
      playlistSource === "playlist" ? videoListPlaylist : videoList;
    const newStateVideos = currentList.map((vidio, i) => {
      if (currentIndex == i && !vidio.isPlaying) {
        return {
          title: vidio.title,
          uri: vidio.uri,
          imageURI: vidio.imageURI,
          time: vidio.time,
          isPlaying: true,
          isDownloaded: vidio.isDownloaded,
          [sourceIsAudio ? "pathAudio" : "pathVideo"]: sourceIsAudio
            ? vidio.pathAudio
            : vidio.pathVideo,
        };
    ...
    if (searchListActive && !videoInfo.isDownloaded) {
      axios
        .get(`https://tubeplaya.herokuapp.com/video_info/${videoInfo.uri}`)
        .then(function (response) {
          let tempRelatedVideos = relatedVideos;
          let isVisited = false;
          if (tempRelatedVideos.length > 0) {
      ...
      if (videoInfo.isDownloaded) {
        changeCurrentVideo(
          sourceIsAudio ? videoInfo.pathAudio : videoInfo.pathVideo
        );
        changeCurrentVideoIndex(newIndex);
        changeCurrentVideoKey(videoInfo.uri);
  ...
  const getSavedSongData = (songData, uriSearch = false) => {
    ...
    axios
      .get(`https://tubeplaya.herokuapp.com/video_info/${uri}`)
      .then(function (response) {
        let isVisited = false;
        let tempRelatedVideos = relatedVideos;
        if (tempRelatedVideos.length > 0) {
  ...
  const playIndexSong = (
    ...
    const currentIndex = currentVideoIndex;
    const videoListLength = isFromPlaylist
      ? videoListPlaylist.length
      : videoList.length;
    const videoInfo = isFromPlaylist
      ? playlistLL[newIndex]
      : videoList[newIndex];
    ...
    axios
      ...
      .then(function (response) {
        ...
        togglePause(false);
        changeRelatedVideos([
          response.data.related_videos[0],
          response.data.related_videos[1],
        ]);
  ...
  const downloadSong = async (
    ...
    let timeInSeconds =
      time.length <= 5 ? Number(time.substring(0, time.length - 3)) : 100;
          ...
          .then(function (response) {
            ...
            AsyncStorage.setItem("Playlists", playlists)
              .then((asyncalive) => {
                ...
                const pathObjectName = sourceIsAudio
                  ? "pathAudio"
                  : "pathVideo";
                ...
                if (
                  imageURI !=
                  `file:///storage/emulated/0/Android/data/com.muustube/files/Images/${uri}.png`
                ) {
                ...
                if (sourceIsAudio) {
                  RNFFmpeg.execute(
                    `-i ${url} file:///storage/emulated/0/Android/data/com.muustube/files/Download/${songObject.uri}.mp3`
                  ).then((result) => {
                    if (result.rc === 0) {
                      songObject[
                        pathObjectName
                      ] = `file:///storage/emulated/0/Android/data/com.muustube/files/Download/${songObject.uri}.mp3`;
            ...
            if (songIsSaved === "Already in playlist") {
              playlistSongs.forEach((song, i) => {
                if (song.uri === uri) {
                  ...
                  if (
                    song["playlist"].indexOf("songsDownloadedOnDevice") === -1
                  )
  ...
  const deleteFromQueue = (uri) => {
    ...
    let tempDownloadingVideoKey = downloadingVideoKey;
    const date = new Date();
    tempDownloadingVideoKey = tempDownloadingVideoKey.filter(
      (dld) => dld != uri
    );
    changeDownloadingVideoKey(tempDownloadingVideoKey);
  };

  const saveSongInLocalStorage = async (
    songObject,
    uri,
  ...
  const updatePlaylistAfterDownload = (songs = null) => {
    if (songs == null) {
      AsyncStorage.getItem("Songs", (e, result) => {
        ...
        let videoListPlaylist = songs.filter((song) =>
          song.playlist.includes(currentPlaylistName)
        );
        if (currentPlaylistName != "songsDownloadedOnDevice") {
          videoListPlaylist.sort(function (a, b) {
            return (
              a.playlistsIndex[currentPlaylistName] -
              b.playlistsIndex[currentPlaylistName]
            );
          });
        }
  ...
  const updateMusicControls = (willPlay = false) => {
    ...
    MusicControl.updatePlayback({
      state: !paused ? MusicControl.STATE_PLAYING : MusicControl.STATE_PAUSED,
      elapsedTime: 135,
      title: videoTitle,
      artwork: imageURI, // URL or RN's image require()
      artist: videoChannel,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#222", position: "relative" }}>
      {splashScreenIsActive ? (
        <SplashScreen
          upliftSplashState={upliftSplashState}
          isAboutToActivate={isAboutToActivate}
        />
      ) : (
