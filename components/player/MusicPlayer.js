...
import { useTogglePause } from "../../redux/actions/storeActions";
import {
  NextButton,
  PlayButton,
  PlayButtonSmall,
...
import SmallPlayer, {
  ...
  VideoInfoContainer,
  VideoTitleContainer,
} from "./components/players/SmallPlayer";
...
const MusicPlayer = (props) => {
  ...
  useEffect(() => {
    ...
    if (!searchListActive) {
      myScroll.scrollTo({
        x: currentVideoIndex * screenWidth,
        y: 0,
        animated: true,
      });
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      currentPlaylistName != "songsDownloadedOnDevice" &&
      currentPlaylistName != "searchbar"
    ) {
      listOfVideos = listOfVideos.filter(
        (video) =>
          video.playlistsIndex != undefined &&
          video.playlistsIndex[currentPlaylistName] != undefined
      );
  ...
  const addToPlaylist = () => {
    ...
    if (video.uri === undefined) {
      let item = {
        ...
        uri: video.id,
        time: 0,
        imageURI: video.video_thumbnail,
      };
  ...
  const artist =
    item === null
      ...
      ? item.videoChannel
        ...
        : item.channel
      : item.customArtist;
  const currentBackgroundImageSource = currentVideoItem.image
    ? currentVideoItem.image
    : currentVideoItem.imageURI;
  // const { relatedVideos, searchListActive, videoListPlaylist, currentPlaylistName, currentVideoIndex, currentVideoKey, allSongs } = props;

  let list_of_videos = searchListActive ? relatedVideos : videoListPlaylist;
  if (
    !searchListActive &&
    list_of_videos.length > 0 &&
    currentPlaylistName != "songsDownloadedOnDevice" &&
    currentPlaylistName != "searchbar"
  ) {
    list_of_videos = list_of_videos.filter(
      (video) =>
        video.playlistsIndex != undefined &&
        video.playlistsIndex[currentPlaylistName] != undefined
    );
  ...
  return (
    <Animated.View
      style={
        fullscreen ? styles.playaContainerFullScreen : styles.playaContainer
      }
    >
      {props.children}
      <Video
      ...
      <SmallPlayer fullscreen={musicPlayerFullScreen}>
        <TouchableOpacity onPress={toggleMusicPlayerFullScreen}>
          <VideoInfoContainer>
            <CurrentVideoThumbnail
              source={currentBackgroundImageSource}
              resizeMode="cover"
            />
            <VideoTitleContainer>
