  ...
  componentDidMount() {
    ...
    AsyncStorage.multiGet(["Playlists", "Songs"], (err, stores) => {
      stores.forEach((store, index) => {
        ...
        if (JSONified != null) {
          if (index == 0) {
            let playlists = JSONified.filter(
              (json) => json != "songsDownloadedOnDevice"
            );
        ...
        {this.state.isLoadingLocalStorage ? (
          ...
          >
            {this.state.displayPlaylists.map((play, index) =>
              play == "songsDownloadedOnDevice" ? null : (
                <TouchableOpacity
                  onLongPress={() => this.openModalEdit(play)}
                  onPress={() =>
    ...
    AsyncStorage.multiGet(["Playlists", "Songs"], (err, stores) => {
      ...
      AsyncStorage.multiSet(
        ...
        () => {
          ...
          this.setState({
            playlists: JSON.parse(jsonPlaylists),
            displayPlaylists: JSON.parse(jsonPlaylists),
          });
        }
      );
    });
